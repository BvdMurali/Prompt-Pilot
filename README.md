...
> **An Enterprise-Grade Prompt Engineering Workspace, Multi-Model LLM Orchestration Engine, and Universal Contextual Rewriting Platform.**

PromptPilot is a high-performance, security-focused web application and backend platform that centralizes the creation, optimization, versioning, and execution of LLM prompts. By pairing a Next.js serverless engine with a Supabase PostgreSQL backend, PromptPilot provides structured prompt refinement, real-time quality grading, template variable bindings, custom API key delegation, and inline web-input injection overlays via downstream browser integrations.
...
The client entry point presents interactive side-by-side prompt comparisons and quality metrics, explaining why and how PromptPilot transforms weak text inputs.
...
### Project Vision
To bridge the gap between unstructured human inputs and instruction-following LLMs. PromptPilot provides an intuitive visual suite and a backend proxy layer that standardizes prompt creation, grading, and testing. It operates as a local utility or cloud platform that eliminates middleman markup fees by allowing users to bring their own API keys.

### The Business & Technical Problem
1. **Prompt Engineering Fragmentation**: Teams manually write, test, and adapt prompts across isolated browser tabs for different providers (OpenAI, Anthropic, Google).
2. **Quality Inconsistency**: Prompts lack role definitions, clear step-by-step formatting, output schemas, or explicit constraints, leading to unpredictable, low-quality LLM completions.
3. **CORS and Auth Synchronization**: Browser extensions attempting inline rewrites on third-party sites hit CORS barriers, session expiration issues, and credentials leakage when querying LLMs directly from client scripts.
4. **Data Privacy & Retention**: Storing proprietary prompts or system credentials requires strict isolation. Users must have full access control, export rights, and permanent soft/hard delete guarantees.

### Core Value Proposition & Differentiators
* **Zero-Markup Gateway**: Executes LLM calls directly using direct API routes or custom API key overrides (Gemini, OpenAI, Anthropic, OpenRouter) configured in dashboard settings.
* **Granular Quality Analytics**: Grades prompts on a 0-100 scale across five architectural dimensions (Clarity, Context, Constraints, Structure, Specificity) with structured JSON feedback explaining the optimization.
* **Privacy-First Soft Delete**: Queues user account deletions for 30 days while offering database-level restoration, protecting multi-tenant integrity.
* **Downstream SDK Integration**: Serves as the central API gateway and authentication hub for the companion Chrome Extension (MV3, Shadow DOM isolated UI) and Expo Mobile App.
...
PromptPilot uses a multi-tier client-server architecture. The web application acts as the authentication coordinator, settings registry, database access layer, and API proxy for external AI gateways.
...
* **Separation of Concerns**: The frontend coordinates the dashboard layouts, Supabase Auth maintains JWT credentials, the server routing handles API orchestration, and PL/pgSQL database triggers enforce schema integrity.
* **Security-First Multitenancy**: PostgreSQL Row-Level Security (RLS) is enabled on all tables, preventing user sessions from reading or writing unauthorized datasets.
* **Graceful Degradation & Fallbacks**: The LLM engine defaults to structured JSON responses. If a provider's model returns raw text or broken schemas, the server sanitizes the string and maps it into standard models.
* **CORS-Free Proxying**: Extension calls go through a background proxy script to the Next.js API, preventing CORS preflight failures on third-party hostnames.

### Design Patterns
* **Service Layer Pattern**: Executed in `web/src/lib/ai.ts` via the `callLLM` function. It abstracts different model formatting protocols (System Messages, user input strings, JSON mode flags) under a single provider call interface.
* **Repository/Direct Access Pattern**: Leverages Supabase Client abstractions for PostgreSQL table interaction, bypassing the need for heavy ORM layers while retaining type-safe SQL-like queries.
* **Real-time Observer Pattern**: Subscribes to changes on the `public.users` table using Supabase Channels. Allows the UI in `dashboard/layout.tsx` to automatically adapt if account deletion triggers are updated or restored.
...
| :--- | :--- | :--- | :--- |
| **Frontend Framework** | Next.js 16.2 (App Router) | Combines client React components with server API routes in a single codebase. | Client dashboard & API routes. |
...
### Authentication Flow
Ensures that the Next.js application, Supabase DB, and downstream extensions remain in sync using JWT tokens and HTML5 cross-domain message events.
...
This monorepo is organized to isolate client-side assets, DB migration files, mobile configurations, and chrome extension logic.

```
/PromptPilot/
├── supabase/                      # Database & Backend Configurations
...
├── assets/                        # Readme visual screenshots
...
│   │   │   ├── globals.css        # Base styling rules
...
Orchestrates the optimization or rewriting of text using LLMs.
